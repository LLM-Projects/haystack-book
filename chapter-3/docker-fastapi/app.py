...
async def upload_file(file: UploadFile = File(...)):
    ...
    with tempfile.NamedTemporaryFile(
        delete=False, mode="wb", suffix=file_extension
    ) as temp_file:
...
async def query(query: Query):
    try:
        ...
        result = querying_pipeline.run(
            data={
                "query_embedder": {"text": query.question},
                "prompt_builder": {"question": query.question},
                "answer_builder": {"query": query.question},
            }
        )
        ...
        return {
            ...
            "details": str(result) if "result" in locals() else "No result available",
        }
